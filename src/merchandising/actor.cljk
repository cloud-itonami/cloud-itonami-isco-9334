(ns merchandising.actor
  "RetailMerchandisingActor — the ISCO-08 9334 community merchandising actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one merchandising operation request
  ...
  The unconditional invariant: the RetailMerchandisingAdvisor can never
  directly commit a record the RetailMerchandisingGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [merchandising.advisor :as advisor]
            [merchandising.governor :as governor]
            [merchandising.store :as store]))

(defn build-graph
  "Build a compiled RetailMerchandisingActor graph. `store` implements
  `merchandising.store/Store`. `advisor` implements
  `merchandising.advisor/Advisor` (defaults to `mock-advisor`).
